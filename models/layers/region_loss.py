...
def build_targets(pred_boxes, target, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen):
    ...
    for b in range(nB):
        ...
        conf_mask=conf_mask.view(-1,cur_ious.size()[0])
        conf_mask[b][cur_ious>sil_thresh] = 0
    conf_mask=conf_mask.view(-1,d1,d2,d3)
    if seen < 12800:
       if anchor_step == 4:
...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        pred_boxes[0] = x.data.view(grid_x.size()) + grid_x
        pred_boxes[1] = y.data.view(grid_y.size()) + grid_y
        pred_boxes[2] = torch.exp(w.data.view(anchor_w.size())) * anchor_w
        pred_boxes[3] = torch.exp(h.data.view(anchor_h.size())) * anchor_h
...
class RegionLossV2(nn.Module):
    ...
    """
    def __init__(self, num_classes=15, anchors=[], num_anchors=1):
    ...
    def forward(self, output, target):
        ...
        anchor_w = anchor_w.repeat(nB, 1).repeat(1, 1, nH*nW).view(nB*nA*nH*nW)
        anchor_h = anchor_h.repeat(nB, 1).repeat(1, 1, nH*nW).view(nB*nA*nH*nW)
        #print("adddddd",x.data.size(),grid_x.size())
        pred_boxes[0] = x.data.view(grid_x.size()) + grid_x
        pred_boxes[1] = y.data.view(grid_y.size()) + grid_y
        pred_boxes[2] = torch.exp(w.data.view(anchor_w.size())) * anchor_w
        pred_boxes[3] = torch.exp(h.data.view(anchor_h.size())) * anchor_h
        ...
        return loss, nCorrect/nGT, nProposals, loss_x.item()+loss_y.item()+loss_w.item()+loss_h.item(), loss_conf.item(), loss_cls.item()
